      ...
      >
        ...
        {blocking && (
          <div className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-3">
            ...
            ולאחר מכן הפעילות תוסר מהרשימה. אם הפעילות לא התקיימה, ניתן לדלג
            והתזכורת תופיע בלוח הבקרה.
          </div>
        )}
        ...
        <div className="flex gap-2 mt-2">
          ...
          <button
            onClick={submit}
            disabled={busy || uploading || skipping}
        ...
        {blocking && onSkip && (
          <button
            ...
            disabled={busy || uploading || skipping}
            className="w-full mt-2 py-2 rounded-xl text-stone-500 hover:text-stone-700 hover:bg-stone-50 font-semibold text-sm disabled:opacity-50"
          >
            {skipping ? 'מדלג…' : 'הפעילות לא התקיימה — דלג/י'}
          </button>
        )}
      </div>
    </div>
